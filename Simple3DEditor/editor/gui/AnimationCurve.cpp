...
AnimationCurve::AnimationCurve(float startFrame, float endFrame) : startFrame(startFrame), endFrame(endFrame) {
    ...
    basicMenu = new Menu();
    basicMenu->AddItem(new MenuItem(L"Ìí¼Óµã", [=]{
    ...
    basicMenu->AddItem(new MenuItem(L"É¾³ýµã", [=]{
        if (this->selIndex > 0)
    ...
    rangeMenu->AddItem(new MenuItem(L"Ë¢ÐÂ·¶Î§", [=]{ this->FlushRange(); }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§2±¶", [=]{ this->ratio *= 0.5f; }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§10±¶", [=]{ this->ratio *= 0.1f; }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§100±¶", [=]{ this->ratio *= 0.01f; }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§0.5±¶", [=]{ this->ratio *= 2.0f; }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§0.1±¶", [=]{ this->ratio *= 10.0f; }));
    rangeMenu->AddItem(new MenuItem(L"·¶Î§0.01±¶", [=]{ this->ratio *= 100.0f; }));
    basicMenu->AddItem(new MenuItem(L"·¶Î§", rangeMenu));

    Menu* funcMenu = new Menu();
    funcMenu->AddItem(new MenuItem(L"³£Á¿", [=]{ this->SetFunc(this->selIndex, leftVal); }));
    funcMenu->AddItem(new MenuItem(L"ÓÒ·½³£Á¿", [=]{ this->SetFunc(this->selIndex, rightVal); }));
    funcMenu->AddItem(new MenuItem(L"ÏßÐÔ", [=]{ this->SetFunc(this->selIndex, linearVal); }));
    funcMenu->AddItem(new MenuItem(L"Æ½·½", [=]{ this->SetFunc(this->selIndex, squareVal); }));
    basicMenu->AddItem(new MenuItem(L"º¯Êý", funcMenu));
}
...
void AnimationCurve::Render(){
    ...
    for (size_t i = 1; i < size; i++){
        ...
        snprintf(s, 40, "(%d,%f)", (int)Floor(p.x), p.y);
