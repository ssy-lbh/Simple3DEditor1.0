...
void Mesh::WriteToOBJ(File file, bool uv, bool normal){
    ...
    if (uv){
        vertices.Foreach([&file](Vertex* v){
            char tmp[MAX_PATH + 1];
            DWORD len;
            len = snprintf(tmp, MAX_PATH, "vt %f %f\n", v->uv.x, v->uv.y);
            file.Write(tmp, len);
        });
    }
    // 顶点法线
    if (normal){
        vertices.Foreach([&file](Vertex* v){
            char tmp[MAX_PATH + 1];
            DWORD len;
            len = snprintf(tmp, MAX_PATH, "vn %f %f %f\n", v->normal.x, v->normal.y, v->normal.z);
            file.Write(tmp, len);
        });
    }
    // 片元
    if (!uv && !normal){
        ...
        faces.Foreach([&file](Face* f){
            ...
            DWORD len;
            int idx1 = f->vertices[0]->index, idx2 = f->vertices[1]->index, idx3 = f->vertices[2]->index;
            len = snprintf(tmp, MAX_PATH, "f %d/%d %d/%d %d/%d\n", idx1, idx1, idx2, idx2, idx3, idx3);
            file.Write(tmp, len);
        });
    }else if (!uv && normal){
        faces.Foreach([&file](Face* f){
        ...
        faces.Foreach([&file](Face* f){
            ...
            DWORD len;
            int idx1 = f->vertices[0]->index, idx2 = f->vertices[1]->index, idx3 = f->vertices[2]->index;
            len = snprintf(tmp, MAX_PATH, "f %d/%d/%d %d/%d/%d %d/%d/%d\n", idx1, idx1, idx1, idx2, idx2, idx2, idx3, idx3, idx3);
            file.Write(tmp, len);
        });
    }
}
